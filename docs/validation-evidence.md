...
Generated by `npm run metrics` on 2026-07-11T21:35:26.243Z. Reproducible: the dataset is
...
| Metric | Value | Detail |
|---|---|---|
| Full detection pass (avg of 5) | **14.1 ms** | entire 14-day dataset, all four detectors |
| Engine throughput | **262,893 tx/sec** | transactions scanned per second |
| Dashboard assembly p95 | **14.1 ms** | management overview + full agent list, 100 runs |
| API read-path avg / p95 | **12.3 / 14.1 ms** | in-process overview + liquidity handler equivalent, 100 runs (network transit excluded) |
