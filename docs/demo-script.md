...
Open **Guided scenarios** from the role picker. Walk the four brief scenarios in order;
...
- **D · Coordinated response and closure** — Ops view opens a high-severity case: who
  receives it, who owns it, the recommended step, the status. Acknowledge → escalate with a
  note → switch to Risk Analyst and resolve. Show the audit trail.
...
Open the bKash operations role. Review the alert queue, evidence, masked balances for other providers, and workflow actions. Acknowledge or escalate with a note.

## 3. Risk Analyst View

Open escalated cases. Review the evidence band and notes, then resolve with a written disposition.

## 4. Management View

Review area hotspots, provider counts, validation metrics, and observability. Management sees aggregates, not individual balances.
